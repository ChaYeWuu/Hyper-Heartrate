...
import com.chayewuu.hyperheartrate.device.ConnectionCallback;
import com.chayewuu.hyperheartrate.device.ConnectionState;
import com.chayewuu.hyperheartrate.device.DeviceFilter;
import com.chayewuu.hyperheartrate.device.DeviceManager;
import com.chayewuu.hyperheartrate.device.DeviceManagerHolder;
import com.chayewuu.hyperheartrate.device.DeviceType;
import com.chayewuu.hyperheartrate.device.ManualBleDevice;
 ...
 * </pre>
 */
public class DeviceSelectionScreen extends BaseModScreen {
    /** 主面板宽度 */
    ...
    /** MAC 输入框 */
    private TextFieldWidget macInputBox;
    ...
    public DeviceSelectionScreen(Screen parent) {
        super(Text.translatable("screen." + HeartRateMod.MOD_ID + ".device_selection"));
        this.parent = parent;
    }
    ...
    protected void init() {
        ...
        macInputBox = new TextFieldWidget(this.textRenderer, controlX + 100, macY, 220, 16, Text.literal("MAC"));
        macInputBox.setMaxLength(17); // AA:BB:CC:DD:EE:FF = 17 字符
        macInputBox.setPlaceholder(Text.literal("AA:BB:CC:DD:EE:FF"));
        addDrawableChild(macInputBox);

        // 手动连接按钮
        addDrawableChild(ButtonWidget.builder(
                Text.literal("连接"),
                btn -> onManualConnectClicked()
        ).dimensions(controlX + 100, macY + 18, 100, 16).build());

        // 重新扫描按钮
        addDrawableChild(ButtonWidget.builder(
                Text.literal("重新扫描"),
                btn -> startScan()
        ).dimensions(controlX, macY + 18, 90, 16).build());

        // 返回按钮
        addDrawableChild(ButtonWidget.builder(
                Text.literal("返回"),
                btn -> close()
        ).dimensions(controlX + 210, macY + 18, 110, 16).build());
    ...
    private void rebuildDeviceButtons() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            ).dimensions(controlX, y, DEVICE_BTN_WIDTH, DEVICE_BTN_HEIGHT).build());
        }
    }
    ...
    private static String formatDeviceLabel(BleDevice device) {
        ...
        String brandLabel = DeviceFilter.getBrandLabel(device.getName());
        return "[" + brandLabel + "] " + name + " (" + addr + ") " + rssi + "dBm";
    }
    ...
    private void onManualConnectClicked() {
        String mac = macInputBox.getText().trim();
        if (!isValidMac(mac)) {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        // 标题
        context.drawCenteredTextWithShadow(this.textRenderer, "选择设备", centerX, panelY + 12, 0xFFFFD700);

        // 扫描状态
        String status = scanning ? "扫描状态: 扫描中..." : "扫描状态: 已停止";
        int statusColor = scanning ? 0xFF66CCFF : 0xFFAAAAAA;
        context.drawCenteredTextWithShadow(this.textRenderer, status, centerX, panelY + 26, statusColor);

        // 分隔线
        int dividerY = panelY + 40 + MAX_DISPLAY_DEVICES * (DEVICE_BTN_HEIGHT + 2) + 4;
        context.fill(panelX + 16, dividerY, panelX + PANEL_WIDTH - 16, dividerY + 1, 0xFF_5A5A7A);

        // MAC 输入提示
        context.drawText(this.textRenderer, "手动输入 MAC:",
        ...
        if (discoveredDevices.isEmpty()) {
            ...
            context.drawCenteredTextWithShadow(this.textRenderer, hint, centerX,
            ...
            if (error != null && !error.isEmpty()) {
                context.drawCenteredTextWithShadow(this.textRenderer, "错误: " + error, centerX,
                        panelY + 40 + MAX_DISPLAY_DEVICES * (DEVICE_BTN_HEIGHT + 2) / 2 + 24, 0xFFFF5555);
                context.drawCenteredTextWithShadow(this.textRenderer, "需安装 .NET 10 运行时: dotnet.microsoft.com", centerX,
