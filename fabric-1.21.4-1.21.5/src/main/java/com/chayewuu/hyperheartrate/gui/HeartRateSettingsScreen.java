 ...
 * </ol>
 */
public class HeartRateSettingsScreen extends BaseModScreen {
    /** 主面板宽度 */
    ...
    protected void init() {
        ...
        addDrawableChild(new CaptureIntervalSlider(controlX, y, CONTROL_WIDTH, CONTROL_HEIGHT,
                config.getCaptureIntervalMs()));
        y += ROW_GAP;
        // 2. 历史数据数量
        addDrawableChild(new HistorySizeSlider(controlX, y, CONTROL_WIDTH, CONTROL_HEIGHT,
                config.getHistorySize()));
        y += ROW_GAP;
        // 3. 自动重连开关
        addDrawableChild(createToggle("自动重连", config.isAutoReconnect(), v -> {
        ...
        y += ROW_GAP;
        // 4. 重连延迟
        addDrawableChild(new ReconnectDelaySlider(controlX, y, CONTROL_WIDTH, CONTROL_HEIGHT,
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.drawCenteredTextWithShadow(this.textRenderer, "心率设置", centerX, panelY + 12, 0xFFFFD700);
    ...
    /** 采集间隔滑块（200~5000ms） */
    private static class CaptureIntervalSlider extends SliderWidget {
        private static final String LABEL = "采集间隔: ";

        CaptureIntervalSlider(int x, int y, int width, int height, int ms) {
            super(x, y, width, height, Text.literal(LABEL + ms + "ms"), valueFromMs(ms));
        }

        @Override
        protected void updateMessage() {
            setMessage(Text.literal(LABEL + msFromValue(this.value) + "ms"));
        }
    ...
    /** 历史数据数量滑块（60~1000） */
    private static class HistorySizeSlider extends SliderWidget {
        private static final String LABEL = "历史数据: ";

        HistorySizeSlider(int x, int y, int width, int height, int size) {
            super(x, y, width, height, Text.literal(LABEL + size), valueFromSize(size));
        }

        @Override
        protected void updateMessage() {
            setMessage(Text.literal(LABEL + sizeFromValue(this.value)));
        }
    ...
    /** 重连延迟滑块（1000~30000ms） */
    private static class ReconnectDelaySlider extends SliderWidget {
