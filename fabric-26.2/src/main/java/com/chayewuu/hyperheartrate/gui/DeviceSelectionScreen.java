...
public class DeviceSelectionScreen extends Screen {
    ...
    protected void init() {
        // 基于内容计算动态高度
        int titleArea = 40;
        int deviceCount = Math.min(discoveredDevices.size(), MAX_DISPLAY_DEVICES);
        int devicesHeight = deviceCount * (DEVICE_BTN_HEIGHT + 2);
        int dividerAndMac = 40;
        int buttonsArea = 40;
        int bottomPadding = 14;
        int calculatedHeight = titleArea + Math.max(devicesHeight, DEVICE_BTN_HEIGHT + 10) + dividerAndMac + buttonsArea + bottomPadding;
        ...
        // MAC 输入框
        int macY = panelY + panelHeight - 70;
     ...
     *
     * @param device 设备
     * @return 形如 "[华为设备] Huawei Watch GT (AA:BB:CC:DD:EE:FF) -60dBm" 的标签
     */
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
        ...
        // 分隔线
        int dividerY = panelY + 40 + Math.min(discoveredDevices.size(), MAX_DISPLAY_DEVICES) * (DEVICE_BTN_HEIGHT + 2) + 4;
        ...
        graphics.text(this.font, "手动输入 MAC:",
                ...
                panelY + panelHeight - 70, 0xFFAAAAAA, false);
