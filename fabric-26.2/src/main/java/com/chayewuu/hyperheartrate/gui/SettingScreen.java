...
public class SettingScreen extends Screen {
    ...
    protected void init() {
        // 计算内容动态高度
        // 控件行数: 1(设备名)+1(位置)+1(字体)+1(透明度)+1(RGB行)+1(预览)+1(心动模式)+1(图标)+1(设备名)+1(BPM)+1(背景板)+1(背景透明度)+1(返回)=13
        int controlRows = 13;
        // 特殊行: RGB行只有24px(非ROW_GAP), 预览行32px(非ROW_GAP), 其余11行用ROW_GAP
        int specialRowsExtra = (24 - ROW_GAP) + (32 - ROW_GAP); // -2+6=4
        int contentOnlyHeight = CONTENT_TOP_OFFSET + ROW_GAP * (controlRows - 1) + specialRowsExtra + CONTROL_HEIGHT + CONTENT_BOTTOM_PADDING;
        ...
        addRenderableWidget(Button.builder(
                ...
                btn -> {
                    ...
                    Minecraft.getInstance().gui.setScreen(new PositionAdjustScreen(this));
                }
        ...
        addRenderableWidget(Button.builder(
                ...
                btn -> {
                    ...
                    Minecraft.getInstance().gui.setScreen(new FontSizeSettingsScreen(this));
                }
        ).bounds(controlX, y, CONTROL_WIDTH, CONTROL_HEIGHT).build());
        y += ROW_GAP;
        // 3. GUI 透明度
        ...
        addRenderableWidget(Button.builder(
                ...
                btn -> {
                    ...
                    Minecraft.getInstance().gui.setScreen(new HeartColorModeScreen(this));
                }
        ...
        y += ROW_GAP;
        // 6c. HUD 背景板开关
        addRenderableWidget(createToggle("HUD 背景板", config.isHudBackgroundEnabled(), v -> {
            config.setHudBackgroundEnabled(v);
            saveConfig();
        }, controlX, y));
        y += ROW_GAP;
        // 6d. HUD 背景板透明度
        addRenderableWidget(new BackgroundOpacitySlider(controlX, y, CONTROL_WIDTH, CONTROL_HEIGHT, config.getHudBackgroundOpacity()));
        y += ROW_GAP;
        // 7. 返回按钮
        addRenderableWidget(Button.builder(
     ...
     * HUD 背景板透明度滑块。
     */
    private static class BackgroundOpacitySlider extends AbstractSliderButton {
