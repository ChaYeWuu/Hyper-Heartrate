 ...
 */
public class SettingScreen extends BaseModScreen {
    ...
    private int panelX;
    private int panelY;
    private TextFieldWidget deviceNameInput;
    ...
    public SettingScreen(Text title, Screen parent) {
        super(title);
        this.parent = parent;
    }
    ...
    protected void init() {
        ...
        // 1. 自定义设备名称输入框
        deviceNameInput = new TextFieldWidget(this.textRenderer, controlX, y, CONTROL_WIDTH, CONTROL_HEIGHT, Text.literal("设备名"));
        deviceNameInput.setMaxLength(32);
        deviceNameInput.setText(config.getCustomDeviceName());
        deviceNameInput.setPlaceholder(Text.literal("自定义设备名称（留空使用真实名）"));
        addDrawableChild(deviceNameInput);
        y += ROW_GAP;
        // 2. 心率显示位置（点击打开位置调整界面）
        addDrawableChild(ButtonWidget.builder(
                Text.literal("心率显示位置（点击拖动调整）"),
                btn -> {
                    ModLogger.info("[SettingScreen] 打开心率显示位置调整界面");
                    MinecraftClient.getInstance().setScreen(new PositionAdjustScreen(this));
                }
        ).dimensions(controlX, y, CONTROL_WIDTH, CONTROL_HEIGHT).build());
        y += ROW_GAP;
        // 2b. 字体大小（点击打开字体大小设置子菜单）
        addDrawableChild(ButtonWidget.builder(
                Text.literal("字体大小（各模块独立设置）"),
                btn -> {
                    ModLogger.info("[SettingScreen] 打开字体大小设置界面");
                    MinecraftClient.getInstance().setScreen(new FontSizeSettingsScreen(this));
                }
        ).dimensions(controlX, y, CONTROL_WIDTH, CONTROL_HEIGHT).build());
        y += ROW_GAP;
        // 3. GUI 透明度
        addDrawableChild(new OpacitySlider(controlX, y, CONTROL_WIDTH, CONTROL_HEIGHT, config.getGuiOpacity()));
        y += ROW_GAP;
        // 4. 字体颜色 R/G/B/A（四个 slider 同一行）
        int sliderX = controlX;
        addDrawableChild(new ColorChannelSlider(sliderX, y, RGB_SLIDER_WIDTH, CONTROL_HEIGHT, 0, rgbaPreview[0], this));
        sliderX += RGB_SLIDER_WIDTH + RGB_SLIDER_GAP;
        addDrawableChild(new ColorChannelSlider(sliderX, y, RGB_SLIDER_WIDTH, CONTROL_HEIGHT, 1, rgbaPreview[1], this));
        sliderX += RGB_SLIDER_WIDTH + RGB_SLIDER_GAP;
        addDrawableChild(new ColorChannelSlider(sliderX, y, RGB_SLIDER_WIDTH, CONTROL_HEIGHT, 2, rgbaPreview[2], this));
        sliderX += RGB_SLIDER_WIDTH + RGB_SLIDER_GAP;
        addDrawableChild(new ColorChannelSlider(sliderX, y, RGB_SLIDER_WIDTH, CONTROL_HEIGHT, 3, rgbaPreview[3], this));
        y += CONTROL_HEIGHT + 4;
        ...
        ButtonWidget heartModeBtn = ButtonWidget.builder(
                Text.literal("心动模式: " + (config.isHeartRateColorMode() ? "开" : "关")),
                b -> {
        ...
        ).dimensions(controlX, y, HEART_MODE_TOGGLE_WIDTH, CONTROL_HEIGHT).build();
        heartModeBtn.setTooltip(net.minecraft.client.gui.tooltip.Tooltip.of(
                Text.literal("开启后心率图标和数值根据心率自动变色：蓝(<60)→绿(60-100)→黄(100-140)→红(>140)")));
        ...
        addDrawableChild(ButtonWidget.builder(
                ...
                btn -> {
                    ...
                    MinecraftClient.getInstance().setScreen(new HeartColorModeScreen(this));
                }
        ).dimensions(controlX + HEART_MODE_TOGGLE_WIDTH + HEART_MODE_BTN_GAP, y,
                HEART_MODE_BTN_WIDTH, CONTROL_HEIGHT).build());
        y += ROW_GAP;
        // 5b. 显示图标
        addDrawableChild(createToggle("显示图标", config.isShowIcon(), v -> {
            config.setShowIcon(v);
            saveConfig();
        }, controlX, y));
        y += ROW_GAP;
        // 6. 显示设备名称
        addDrawableChild(createToggle("显示设备名称", config.isShowDeviceName(), v -> {
            config.setShowDeviceName(v);
            saveConfig();
        }, controlX, y));
        y += ROW_GAP;
        // 6b. 显示 BPM 后缀
        addDrawableChild(createToggle("显示 BPM 后缀", config.isShowBpmSuffix(), v -> {
            config.setShowBpmSuffix(v);
            saveConfig();
        }, controlX, y));
        y += ROW_GAP;
        // 6c. HUD 背景板开关
        addDrawableChild(createToggle("HUD 背景板", config.isHudBackgroundEnabled(), v -> {
            config.setHudBackgroundEnabled(v);
            saveConfig();
        }, controlX, y));
        y += ROW_GAP;
        // 6d. HUD 背景板透明度
        addDrawableChild(new BackgroundOpacitySlider(controlX, y, CONTROL_WIDTH, CONTROL_HEIGHT, config.getHudBackgroundOpacity()));
        y += ROW_GAP;
        // 7. 返回按钮
        addDrawableChild(ButtonWidget.builder(
                Text.literal("返回"),
    ...
    private void saveDeviceNameIfChanged() {
        if (deviceNameInput != null) {
            ModConfig config = ConfigManager.getConfig();
            String newName = deviceNameInput.getText().trim();
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        // 标题（不裁剪，始终可见）
        if (guiScale != 1.0f) {
            org.joml.Matrix3x2fStack stack = context.getMatrices();
            stack.pushMatrix();
            stack.translate(centerX, panelY + 12);
            stack.scale(guiScale, guiScale);
            ModFontManager.centeredText(context, modFont, "GUI 设置", 0, 0, 0xFFFFD700);
            stack.popMatrix();
        } else {
            ModFontManager.centeredText(context, modFont, "GUI 设置", centerX, panelY + 12, 0xFFFFD700);
        }
        ...
        // 预览背景（边框）
        context.fill(previewX - 1, previewY - 1, previewX + previewWidth + 1, previewY + PREVIEW_HEIGHT + 1, 0xFF444444);
        // 预览内容（RGBA 颜色）
        int argb = colorToArgb(rgbaPreview, 1.0f);
        context.fill(previewX, previewY, previewX + previewWidth, previewY + PREVIEW_HEIGHT, argb);
        // 预览标签（居中显示在色块上）
        ...
        context.drawCenteredTextWithShadow(this.textRenderer, previewLabel, centerX, previewY + (PREVIEW_HEIGHT - 8) / 2, labelColor);
    ...
    private static class OpacitySlider extends SliderWidget {
        ...
        OpacitySlider(int x, int y, int width, int height, float value) {
            super(x, y, width, height, Text.literal(LABEL + percent(value)), clamp01(value));
        }

        @Override
        protected void updateMessage() {
            setMessage(Text.literal(LABEL + percent((float) this.value)));
        }
     ...
     * HUD 背景板透明度滑块。
     */
    private static class BackgroundOpacitySlider extends SliderWidget {
        private static final String LABEL = "背景板透明度: ";

        BackgroundOpacitySlider(int x, int y, int width, int height, float value) {
            super(x, y, width, height, Text.literal(LABEL + percent(value)), clamp01(value));
        }
        ...
        protected void applyValue() {
            ModConfig config = ConfigManager.getConfig();
            config.setHudBackgroundOpacity((float) clamp01(this.value));
            ConfigManager.save();
        }
     ...
     * 字体颜色通道滑块。滑动时同时更新预览。
     */
    private static class ColorChannelSlider extends SliderWidget {
        ...
        ColorChannelSlider(int x, int y, int width, int height, int channel, float normalized, SettingScreen parentScreen) {
            super(x, y, width, height, Text.literal(LABELS[channel] + ":" + toInt(normalized)), clamp01(normalized));
        ...
        @Override
        protected void updateMessage() {
            setMessage(Text.literal(LABELS[channel] + ":" + toInt(this.value)));
        }
