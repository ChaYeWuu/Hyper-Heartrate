 ...
 * Redmi Watch 心率解析器。
 * <p>
 * Redmi Watch 系列通常兼容标准 GATT Heart Rate Service（0x180D），
 * 心率数据通过标准 0x2A37 特征值通知上报。本解析器复用标准解析逻辑，
 * 并为后续可能的 Redmi 私有协议扩展预留扩展点。
 * </p>
 *
 * <p><b>路由优先级：</b>高于 {@link StandardGattHeartRateParser}，
 * 当设备类型为 {@link DeviceType#REDMI_WATCH} 时命中。</p>
 *
 * <p><b>线程安全：</b>解析器无状态，内部持有的 {@link StandardGattHeartRateParser}
 * 同样无状态，可被多线程并发调用。</p>
 */
public class RedmiParser implements HeartRateParser {
    ...
    public int parseHeartRate(byte[] data) {
        // Redmi Watch 兼容标准 GATT 心率服务，直接复用标准解析逻辑
        // 若后续需要支持 Redmi 私有协议，可在此处根据数据特征分支处理
    ...
    public boolean canParse(BleDevice device) {
        ...
        return device.getType() == DeviceType.REDMI_WATCH;
    }
}
