...
public class HeartRateHudWidget {
    ...
    private void renderUnified(DrawContext context, int screenWidth, int screenHeight, ModConfig config) {
        ...
        // 心率图标颜色：心动模式且开启图标变色
        int iconColor = (colorMode && config.isHeartColorIcon()) ? heartColor : 0xFFFF4060;
        // 心率数值颜色
        ...
        if (showDevice) {
            // 设备名行（不变色）
            stack.push();
            stack.translate(posX + padding, posY + padding, 0);
            stack.scale(dnScale, dnScale, 1);
        ...
        if (splitRender) {
            ...
            stack.push();
            stack.translate(textStartX, hrRowY, 0);
            stack.scale(hrScale, hrScale, 1);
            context.drawText(font, rateStr, 0, 0, rateColor, true);
            stack.pop();
            stack.push();
            stack.translate(textStartX + (int)(rateWidth * hrScale), hrRowY, 0);
            stack.scale(hrScale, hrScale, 1);
    ...
    private void renderModular(DrawContext context, int screenWidth, int screenHeight, ModConfig config) {
        ...
        // 心率文字模块
        {
            ...
            if (splitRender) {
                ...
                stack.pop();
                stack.push();
                stack.translate(tx + (int)(rateWidth * hrScale), ty, 0);
                stack.scale(hrScale, hrScale, 1);
                context.drawText(font, bpmStr, 0, 0, bpmColor, true);
                stack.pop();
            } else {
