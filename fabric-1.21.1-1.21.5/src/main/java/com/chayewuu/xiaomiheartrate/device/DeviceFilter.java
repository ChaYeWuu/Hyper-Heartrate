 ...
 * 设备类型识别工具类。
 * <p>
 * 所有支持心率广播的设备均会被上报，非小米/Redmi 设备统一归类为 STANDARD_GATT（通用心率广播设备），通过标准 GATT 0x2A37 心率服务解析。
 * </p>
 *
 * <p>该类为工具类，禁止实例化。</p>
 *
...
public final class DeviceFilter {
    ...
    /**
     * 判断设备是否为支持心率广播的设备。
     * <p>移除白名单后，所有非 null 设备均视为支持。</p>
     *
     * @param device 待判定设备
     * @return {@code true} 表示该设备支持心率广播（非 null 即返回 true）
     */
    ...
    /**
     * 根据设备名称推断设备类型。
     * <p>匹配方式为大小写不敏感的前缀匹配。无法识别的名称统一归类为
     * {@link DeviceType#STANDARD_GATT}（通用心率广播设备）。</p>
     *
     * @param name 设备名称
     * @return 设备类型；无法识别时返回 {@link DeviceType#STANDARD_GATT}
     */
