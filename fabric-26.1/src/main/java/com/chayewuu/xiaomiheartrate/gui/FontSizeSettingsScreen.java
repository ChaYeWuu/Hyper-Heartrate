...
public class FontSizeSettingsScreen extends Screen {
    ...
    public FontSizeSettingsScreen(Screen parent) {
        super(Component.literal("字体大小设置"));
        this.parent = parent;
    }
    ...
    protected void init() {
        ...
        // 4. 心率图标缩放
        addRenderableWidget(new IntScaleSlider(
                controlX, y, CONTROL_WIDTH, CONTROL_HEIGHT,
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
        ...
        graphics.centeredText(ModFontManager.getFont(), "字体大小设置", centerX, panelY + 12, 0xFFFFD700);
    ...
    private static class ScaleSlider extends AbstractSliderButton {
        ...
        ScaleSlider(int x, int y, int w, int h, String label, double current, double min, double max,
            ...
            super(x, y, w, h, Component.literal(label + ": " + pct(current)), toNormalized(current, min, max));
            this.label = label;
            this.min = min;
            this.max = max;
        ...
        protected void updateMessage() {
            setMessage(Component.literal(label + ": " + pct(getValue())));
        }
     ...
     * 整数缩放滑块（倍数显示）。
     */
    private static class IntScaleSlider extends AbstractSliderButton {
        private final String label;
        ...
        IntScaleSlider(int x, int y, int w, int h, String label, int current, int min, int max,
            ...
            super(x, y, w, h, Component.literal(label + ": " + current + "x"), toNormalized(current, min, max));
            this.label = label;
            this.min = min;
            this.max = max;
            this.onChange = onChange;
        ...
        @Override
        protected void updateMessage() {
            setMessage(Component.literal(label + ": " + getValue() + "x"));
        }
